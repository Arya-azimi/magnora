...
export default function PrimarySubmitButton({
  ...
  return (
    <button type="submit" className={`${className} inline-block group`} aria-label="Learn more">
      {/* wrapper */}
      <div
      ...
      >
        {/* 1. GHOST ELEMENT (اندازه‌گیر نامرئی برای حفظ سایز دقیق دکمه) */}
        <div className="pointer-events-none flex h-full items-center opacity-0" aria-hidden="true">
          <div className="w-[60px] shrink-0" />
          <span className={`button-text ${buttonTextClass} shrink-0 stroke-0 px-4 text-nowrap`}>
            {buttonText}
          </span>
        </div>

        {/* 2. ICON TRACK & ICON (با متد Pusher Spacer) */}
        <div className="absolute inset-y-1 left-1 right-1 z-20 flex pointer-events-none" aria-hidden="true">
          {/* === SPACER === */}
        ...
        <div className="absolute left-[64px] right-1 flex h-full items-center justify-center transition-all duration-700 ease-in-out group-hover:left-1 group-hover:right-[64px]">
          <span
            className={`button-text ${buttonTextClass} shrink-0 stroke-0 px-4 text-nowrap`}
            aria-hidden="true"
          >
            {buttonText}
          </span>
